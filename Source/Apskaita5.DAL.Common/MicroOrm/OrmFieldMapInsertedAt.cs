...
namespace Apskaita5.DAL.Common.MicroOrm
{
    ...
    public sealed class OrmFieldMapInsertedAt<T> : OrmFieldMapBase<T> where T : class
    {
        ...
        public OrmFieldMapInsertedAt(string dbFieldName, string propName, Action<T, DateTime> valueSetter, Func<T, DateTime> valueGetter)
            : base(dbFieldName, propName, FieldPersistenceType.InsertOnly, null, false)
        {
        ...
        internal void InitValue(T instance, OrmFieldMapUpdatedAt<T> updatedAtFieldMap)
        {
            if (updatedAtFieldMap.IsNull()) throw new ArgumentNullException(nameof(updatedAtFieldMap));

            var value = Utilities.GetCurrentTimeStamp();
            ValueSetter(instance, value);
            updatedAtFieldMap.ValueSetter(instance, value);   
        }
