...
namespace Apskaita5.DAL.Common.MicroOrm
{
    ...
    public sealed class OrmFieldMapByteArray<T> : OrmFieldMapBase<T> where T : class
    {

        /// <summary>
        /// create a new description how a business object byte[] property (field) is persisted in a database;
        /// </summary>
        ...
        public OrmFieldMapByteArray(string dbFieldName, string propName, Action<T, byte[]> valueSetter, Func<T, byte[]> valueGetter,
        ...
        /// <summary>
        /// create a new description how a business object readonly byte[] property (field) is persisted in a database;
        /// </summary>
        /// <param name="dbFieldName">a name of the database table field that the property value is persisted in</param>
        /// <param name="propName">a name of the the property that the value is managed by; match LightDataColumn name 
        /// that is returned on business object field</param>
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public OrmFieldMapByteArray(string dbFieldName, string propName, Action<T, byte[]> valueSetter, bool isInitializable = false)
            : this(dbFieldName, propName, valueSetter, null, FieldPersistenceType.Readonly, null, isInitializable) { }


        private Func<T, byte[]> ValueGetter { get; }
        private Action<T, byte[]> ValueSetter { get; }
