...
{
    /// <summary>
    /// describes how a business object float property (field) is persisted in a database;
    ...
    public sealed class OrmFieldMapFloat<T> : OrmFieldMapBase<T> where T : class
    {

        /// <summary>
        /// create a new description how a business object float property (field) is persisted in a database;
        /// </summary>
        ...
        public OrmFieldMapFloat(string dbFieldName, string propName, Action<T, float?> valueSetter, Func<T, float?> valueGetter,
        ...
        /// <summary>
        /// create a new description how a business object readonly float property (field) is persisted in a database;
        /// </summary>
        /// <param name="dbFieldName">a name of the database table field that the property value is persisted in</param>
        /// <param name="propName">a name of the the property that the value is managed by; match LightDataColumn name 
        /// that is returned on business object field</param>
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public OrmFieldMapFloat(string dbFieldName, string propName, Action<T, float?> valueSetter, bool isInitializable = false)
            : this(dbFieldName, propName, valueSetter, null, FieldPersistenceType.Readonly, null, isInitializable) { }


        private Func<T, float?> ValueGetter { get; }
        private Action<T, float?> ValueSetter { get; }
        ...
        {
            ValueSetter(instance, row.GetFloatNullable(PropName));
        }
